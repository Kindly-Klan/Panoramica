		...
		try {
			...
			if (Keybinds.keybindCreatePano.isDown()) {
				...
				if (!mc.getWindow().isFullscreen()) {
					...
					Minecraft.getInstance().player.sendSystemMessage(Component.literal("§cPlease disable fullscreen mode before creating a panorama!"));
				}
	...
	private boolean finishPanorama() {
		try {
			if (this.screenshots.size() == 6) {
				...
				Minecraft.getInstance().player.sendSystemMessage(Component.literal("§cError! Unable to save panorama!"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
